...
<head>
    ...
    <script>
        function getLandscapeDetailed(id) {
            var url = "getLandscapeDetailed.php";
            var postData = {'nature_id': id};
            var dataType = "json";


            function myFunc(data) {
                document.getElementById('description').innerHTML = data.descr;
            ...
            <div class="row-fluid">
                ...
                <div class="span6">
                    <ul class="breadcrumb pull-right">
                        ...
                        <li class="active" style="color: yellow;">Landscape</li>
                    </ul>
                </div>
            </div>
        </div>
...
<section class="content">
    <div class="row-fluid">
        ...
        if ($conn) {
            $sql = "select nature_id, title, description, mainImgPath from nature";

            if ($result = mysql_query($sql, $conn)) {
                while ($row = mysql_fetch_array($result)) {
                    echo '<div class="span3">';
                    echo '<div class="box">';
                    echo '<p><img src="' . $row['mainImgPath'] . '" style="height:250px; width:100%;"></p>';
                    echo '<h2 style="text-align:center;">' . $row['title'] . '</h2>';
                    echo '<button class="btn btn-large btn-info open_popup" style="width: 100%;"
                                    onclick="getLandscapeDetailed(' . $row['nature_id'] . ')">';
                    echo 'View';
                    echo '</button>';
                    echo '</div>';
                    echo '</div>';
